...
{ 
    ...
    float ACCUMULATED_FRAMES_THRESH = AGGRESSIVE_DISOCCLUSION_HANDLING ? 4.0f+4.0f+FRAME_BIAS : 4.0f+FRAME_BIAS;

    if (ACCUMULATED_FRAMES < ACCUMULATED_FRAMES_THRESH) 
    {
        float ColorPhi = AGGRESSIVE_DISOCCLUSION_HANDLING ? 5.0f : 5.0f*2.0f; 
