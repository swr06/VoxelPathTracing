...
void SpatiallyUpscaleBuffers(vec3 BaseNormal, vec3 BaseHFNormal, float BaseRoughness, float BaseLinearDepth, out vec4 SH, out vec2 CoCg, out vec4 SpecularIndirect, out float ShadowSample, bool fuckingsmooth, out float ao)
{
	...
	TotalShadow  += 0.05f;
	TotalShadow = clamp(TotalShadow, 0.0f, 1.0f);
...
void main()
{
    ...
    if (WorldPosition.w > 0.0f)
    {
        ...
        if (true)
        {
            ...
            if (InBiasedSS) {
                SpecularIndirect = UpscaledSpecularIndirect.xyz;
            }
            ...
            if (!dfg) {
				...
				SpecularAO = clamp(pow(SpecularAO, 1.5f),0.,1.);
				
				// Light leak handling
				float HorizonAmount = min(1.0f + dot(R, NormalMapped), 1.0f);
				HorizonAmount *= HorizonAmount * HorizonAmount;
                HorizonAmount = clamp(HorizonAmount, 0.5, 1.);
				
				SpecularIndirect *= min(HorizonAmount * 0.94f, 1.);
